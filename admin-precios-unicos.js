...
function vigenciasIdsEquipo(equipoId){
    return [...new Set(DATA.precios.filter(p=>p.equipo_id===equipoId && p.vigencia_id).map(p=>p.vigencia_id))];
}
...
async function guardarVigenciaEquipo(equipoId,fechaFin){
    const ids=vigenciasIdsEquipo(equipoId);
    if(ids.length){
        for(const vigenciaId of ids){
            await adminCall('update','vigencias',{id:vigenciaId,payload:{fecha_fin:fechaFin||null,activa:true}});
        }
        return;
    }
    if(!fechaFin) return;
    const nombre='Vigencia '+(DATA.equipos.find(e=>e.id===equipoId)?.nombre||equipoId);
    const creada=await adminCall('insert','vigencias',{payload:{nombre,fecha_inicio:null,fecha_fin:fechaFin,activa:true}});
    const vigenciaId=creada?.data?.[0]?.id;
    if(vigenciaId){
        await adminCall('update_filters','precios',{filters:{equipo_id:equipoId},payload:{vigencia_id:vigenciaId}});
    }
}
function equipoForm(e,isNew){const specs=specsToLines(e?.especificaciones||{});const sell=normalizarListaEditor(e?.argumentos_venta).join('\n');const obj=objecionesToLines(e?.objeciones||[]);const statusComercial=e?.datos_extra?.status || (e?.activo!==false?'RESURTIBLE':'NO RESURTIBLE');const vigenteHasta=e?.id?vigenteHastaEquipo(e.id):'';let html=`<div class="grid">${field('Código','eq_codigo',e?.codigo||'', 'text',isNew?'':'readonly')}${field('Nombre','eq_nombre',e?.nombre||'')}${field('Marca','eq_marca',e?.marca||'')}${field('Capacidad','eq_capacidad',e?.capacidad||'')}<div><label>Estado en catálogo</label><select id="eq_activo"><option value="1" ${e?.activo!==false?'selected':''}>Disponible</option><option value="0" ${e?.activo===false?'selected':''}>No disponible</option></select></div><div><label>Status comercial en cotización</label><select id="eq_status"><option value="RESURTIBLE" ${statusComercial==='RESURTIBLE'?'selected':''}>RESURTIBLE</option><option value="NO RESURTIBLE" ${statusComercial==='NO RESURTIBLE'?'selected':''}>NO RESURTIBLE</option></select></div>${field('Vigente hasta','eq_vigente_hasta',vigenteHasta,'date')}${field('URL de imagen','eq_imagen',e?.imagen_url||'')}</div><div class="subcard"><h3>Imagen</h3><div class="row"><input id="eq_file" type="file" accept="image/*" style="max-width:430px"><button onclick="subirImagenEquipo('${esc(e?.id||'')}')">Subir imagen</button>${e?.imagen_url?`<img class="preview" src="${esc(e.imagen_url)}">`:''}</div><div class="note">Se sube al bucket <b>equipos</b> y se guarda la URL automáticamente.</div></div><div class="grid">${textarea('Características (una por línea)','eq_specs',specs,'Puedes escribir por ejemplo: Pantalla: AMOLED 6.9&quot; o simplemente una característica por línea.')}${textarea('Argumentos de venta (uno por línea)','eq_sell',sell)}${textarea('Objeciones (una por línea)','eq_obj',obj,'Se mostrará igual que en el cotizador: Pregunta: ... | Respuesta: ...')}</div><div class="row" style="margin-top:18px"><button class="primary" onclick="guardarEquipo('${esc(e?.id||'')}')">💾 Guardar equipo</button>${!isNew?`<button class="danger" onclick="eliminarEquipo('${esc(e.id)}')">Eliminar definitivamente</button>`:''}</div>`;
...
if(equipoId) await guardarVigenciaEquipo(equipoId,val('eq_vigente_hasta')||null);
await reload();cerrarModal();msg('✓ Equipo y vigencia guardados de forma segura','success');
...
for(const i of inputs){
    if(!i.dataset.plazo) continue;

    const precio=i.value.trim()===''?null:Number(i.value);
    const filters={
        equipo_id:equipoId,
        plan_id:i.dataset.plan,
        plazo_id:i.dataset.plazo
    };

    // El Admin es la única fuente de verdad: primero elimina cualquier
    // registro anterior de esta combinación equipo + plan + plazo.
    await adminCall('delete_filters','precios',{filters});

    // Si hay un precio capturado, crea exactamente un registro nuevo.
    if(precio!==null){
        await adminCall('insert','precios',{payload:{
            equipo_id:equipoId,
            plan_id:i.dataset.plan,
            plazo_id:i.dataset.plazo,
            precio,
...
await reload();
alert('Precios guardados. El Admin es ahora la única fuente de precios para este equipo.');
}catch(e){
    alert('No se pudieron guardar precios: '+e.message);
}};
