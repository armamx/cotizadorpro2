...
let _variants=(window.STORAGE_VARIANTS || {})[id] || null;
  ...
  h+='<button onclick="enviarCotizacion()" class="wa-btn" style="width:100%;padding:13px 18px;background:#00A8E0;color:#fff;font-size:14px;font-weight:600;border:none;border-radius:12px;cursor:pointer;display:flex;align-items:center;justify-content:center;letter-spacing:.8px;font-family:Inter,system-ui,-apple-system,Segoe UI,Arial,sans-serif;box-shadow:0 6px 16px rgba(0,168,224,.18);transition:all .2s ease">';
  h+='GENERAR COTIZACION <svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" style="margin-left:8px"><circle cx="12" cy="12" r="8.5"></circle><path d="M14.8 8.7c-.7-.6-1.7-.9-2.8-.9-1.7 0-2.9.8-2.9 2s1 1.7 3 2.1c2 .4 3 1 3 2.2 0 1.3-1.2 2.2-3.1 2.2-1.2 0-2.4-.4-3.2-1.1"></path><path d="M12 6.3v11.4"></path></svg></button>';
  h+='</div></div>';
...
function enviarCotizacion(){
  try{
    ...
    alert('No se pudo abrir el generador de cotización. Revisa la consola para ver el error exacto.');
  }
}
...
const isUpcoming=
  ...
  const _devInfo = document.getElementById('cot-device-info');
  if(_devInfo) _devInfo.textContent = _subtitle;
  ...
  const _segDesc = document.getElementById('cot-seguro-desc');
  if(_segDesc) _segDesc.textContent = '$'+seguroPrice+'/mes (según precio del equipo)';
  ...
  const _engWrap=document.getElementById('cot-eng-custom-wrap');
  if(_engWrap) _engWrap.classList.remove('show');
  const _engInput=document.getElementById('cot-eng-custom-input');
  if(_engInput) _engInput.value='';
  
  cotRender();
  const _overlay=document.getElementById('cot-overlay');
  if(!_overlay) throw new Error('No existe #cot-overlay en el HTML.');
...
function cotRender(){
  // Read custom input if applicable. Estos controles pueden estar ocultos o
  // retirados del HTML; en ese caso se conserva enganche = 0.
  const _engWrapRender=document.getElementById('cot-eng-custom-wrap');
  const _engInputRender=document.getElementById('cot-eng-custom-input');
  if(_engWrapRender && _engInputRender && _engWrapRender.classList.contains('show')){
    const v = parseFloat(_engInputRender.value)||0;
    cotState.engCustom = v;
  }
  // [v1.8.2] Leer depósito de garantía si está activo
  const depWrap = document.getElementById('cot-deposito-custom-wrap');
  if(depWrap && depWrap.classList.contains('show')){
    const _depInputRender=document.getElementById('cot-deposito-input');
    const v = _depInputRender ? (parseFloat(_depInputRender.value)||0) : 0;
    cotState.deposito = v;
  } else {
    cotState.deposito = 0;
  }
...
if(cotState.port){
  ...
  const _resumen=document.getElementById('cot-resumen');
  if(_resumen) _resumen.innerHTML = h;
