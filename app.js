...
const totalMensualSinPort =
 ...
 if(cotState.port){
  ...
  msg += '✅ *¿Te gustó esta opción?* ¡Avancemos hoy al siguiente paso!';
...
function buildFlyerMultiHTML(state){
  ...
  cols.forEach(function(c){
    ...
    if(state.port){
      h+='<div style="font-size:9px;color:#1A8038;font-weight:600;margin-top:3px;line-height:1.3">Con portabilidad<br>$'+_flyerFmx(c.totalMensualPort)+'/mes en facturas 2 a 7</div>';
    }
    h+='</div></div>';
  });
  h+='</div>';
