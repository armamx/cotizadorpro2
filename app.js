...
let _variants=(window.STORAGE_VARIANTS || {})[id] || null;
  ...
  h+='<button type="button" onclick="enviarCotizacion()" class="wa-btn" style="width:100%;padding:14px;color:#fff;background:linear-gradient(135deg,#0AADEB 0%,#087FCB 100%);font-size:15px;font-weight:800;letter-spacing:.4px;border:none;border-radius:var(--r-xl);cursor:pointer;display:flex;align-items:center;justify-content:center;box-shadow:0 7px 18px rgba(0,153,214,.24)">';
  h+='GENERAR COTIZACION</button>';
  h+='</div></div>';
...
function enviarCotizacion(){
  try{
    ...
    console.error('[cotizacion] No se pudo abrir el generador:', e);
    alert('No se pudo abrir el generador de cotización. Recarga la app e inténtalo de nuevo.');
  }
}
...
const isUpcoming=
  ...
  const _devInfoEl=document.getElementById('cot-device-info');
  if(_devInfoEl) _devInfoEl.textContent = _subtitle;
  ...
  // Reset toggles
  const _portSw=document.getElementById('cot-port-switch'); if(_portSw) _portSw.classList.remove('on');
  const _ctrlSw=document.getElementById('cot-control-switch'); if(_ctrlSw) _ctrlSw.classList.remove('on');
  const _segSw=document.getElementById('cot-seguro-switch'); if(_segSw) _segSw.classList.remove('on');
  const _engWrap=document.getElementById('cot-eng-custom-wrap'); if(_engWrap) _engWrap.classList.remove('show');
  const _engInp=document.getElementById('cot-eng-custom-input'); if(_engInp) _engInp.value='';
  
  cotRender();
  const _cotOverlay=document.getElementById('cot-overlay');
  if(!_cotOverlay){ throw new Error('Falta #cot-overlay en index.html'); }
