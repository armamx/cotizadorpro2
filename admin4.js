...
window.guardarEquipo=async id=>{try{const codigo=val('eq_codigo'),nombre=val('eq_nombre');if(!codigo||!nombre)throw new Error('Código y nombre son obligatorios.');const payload={codigo,nombre,marca:val('eq_marca'),capacidad:val('eq_capacidad'),activo:val('eq_activo')==='1',imagen_url:val('eq_imagen')||null,especificaciones:parseSpecsLines('eq_specs'),argumentos_venta:parseListaEditor('eq_sell'),objeciones:parseListaEditor('eq_obj')};let r=id?await db.from('equipos').update(payload).eq('id',id).select().single():await db.from('equipos').insert(payload).select().single();if(r.error)throw r.error;await reload();cerrarModal();msg('✓ Equipo guardado en Supabase','success')}catch(e){alert('No se pudo guardar: '+(e.message||e))}};
