...
async function guardarVigenciaEquipo(equipoId,fechaFin){
    const equipo=DATA.equipos.find(e=>e.id===equipoId);
    let vigenciaId=vigenciasIdsEquipo(equipoId)[0] || null;

    if(vigenciaId){
        await adminCall('update','vigencias',{
            id:vigenciaId,
            payload:{fecha_fin:fechaFin||null,activa:true}
        });
    }else{
        if(!fechaFin) return null;

        const nombre='Vigencia '+(equipo?.nombre||equipoId);
        ...
        vigenciaId=creada?.data?.[0]?.id || null;
    }

    // Mantener enlazados los precios existentes, sin usar update_filters.
    if(vigenciaId){
        const preciosEquipo=DATA.precios.filter(p=>p.equipo_id===equipoId);
        for(const p of preciosEquipo){
            if(p.vigencia_id!==vigenciaId){
                await adminCall('update','precios',{
                    id:p.id,
                    payload:{vigencia_id:vigenciaId}
                });
            }
        }
    }
...
if(equipoId){
    ...
    // Guardar el vínculo de vigencia en el propio equipo para no depender
    // de que ya existan precios.
    if(vigenciaId){
        const extraFinal={...datosExtra,vigencia_id:vigenciaId};
