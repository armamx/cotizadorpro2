...
async function guardarVigenciaEquipo(equipoId,fechaFin){
    ...
    // La relación principal ahora vive directamente en equipos.datos_extra.vigencia_id.
    // Ya no dependemos de la tabla precios para localizar la vigencia.
    const vigenciaId=equipo?.datos_extra?.vigencia_id || null;

    if(!vigenciaId){
        throw new Error('Este equipo no tiene vigencia vinculada. Revisa datos_extra.vigencia_id.');
    }

    // Actualizar exclusivamente la vigencia ya vinculada.
    ...
    // Actualizar la copia local para que la interfaz refleje el cambio.
