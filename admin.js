    ...
    return lista.map(function(item){
        ...
        if(item && typeof item === 'object'){
            if('q' in item || 'a' in item){
                const q = item.q ?? '';
                const a = item.a ?? '';
                if(q && a) return 'Pregunta: ' + q + ' | Respuesta: ' + a;
...
function parseObjecionesEditor(id){
    return lines(id).map(function(linea){
        const sepRespuesta = ' | Respuesta: ';
        const sepPregunta = ' | Pregunta: ';

        if(linea.startsWith('Pregunta: ')){
            const cuerpo = linea.slice('Pregunta: '.length);
            const pos = cuerpo.indexOf(sepRespuesta);
            if(pos >= 0){
                return {
                    q: cuerpo.slice(0,pos).trim(),
                    a: cuerpo.slice(pos + sepRespuesta.length).trim()
                };
            }
        ...
        if(linea.startsWith('Respuesta: ')){
            ...
            const pos = cuerpo.indexOf(sepPregunta);
            if(pos >= 0){
                return {
                    q: cuerpo.slice(pos + sepPregunta.length).trim(),
                    a: cuerpo.slice(0,pos).trim()
                };
            }
            return {q:'', a:cuerpo.trim()};
        }
...
const fechaFin=val('eq_vigente_hasta')||null;
if(equipoId){
    const vigIds=vigenciasIdsEquipo(equipoId);
    for(const vigenciaId of vigIds){
        await adminCall('update','vigencias',{
            id:vigenciaId,
            payload:{fecha_fin:fechaFin,activa:true}
        });
    }
}
...
if(!url)throw new Error('La función no devolvió la URL de la imagen.');
await reload();
const actualizado=DATA.equipos.find(x=>x.id===id);
if(actualizado)open('Editar equipo',equipoForm(actualizado,false));
